...
router.get('/', async (req, res) => {
  ...
  const users = await UserModel.find(query).select('-passwordHash -passwordPlain');
  res.json(users);
});
...
router.post('/', validate(createUserSchema), async (req, res) => {
  ...
  try {
    ...
    const { passwordHash: _, passwordPlain: __, ...safe } = user.toJSON();
...
router.put('/:id', validate(updateUserSchema), async (req, res) => {
  ...
  const updated = await UserModel.findByIdAndUpdate(req.params.id, update, { new: true }).select('-passwordHash -passwordPlain');
