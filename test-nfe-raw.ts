...
async function main() {
  ...
  const soap = `<?xml version="1.0" encoding="UTF-8"?><env:Envelope xmlns:env="http://www.w3.org/2003/05/soap-envelope" xmlns:nfe="http://www.portalfiscal.inf.br/nfe/wsdl/NFeAutorizacao4"><env:Header/><env:Body><nfe:nfeDadosMsg><enviNFe xmlns="http://www.portalfiscal.inf.br/nfe" versao="4.00"><idLote>${Date.now()}</idLote><indSinc>1</indSinc>${signed}</enviNFe></nfe:nfeDadosMsg></env:Body></env:Envelope>`;

  const https = require('https');
  const resp = await axios.post(
    'https://homologacao.nfe.fazenda.sp.gov.br/ws/nfeautorizacao4.asmx',
    soap,
    { headers: { 'Content-Type': 'application/soap+xml; charset=utf-8; action="http://www.portalfiscal.inf.br/nfe/wsdl/NFeAutorizacao4/nfeAutorizacaoLote"' },
      httpsAgent: new https.Agent({ key: keyPem, cert: certPem, rejectUnauthorized: false }), timeout: 30000 }
  );
...
main().catch(e => console.error('ERRO:', e.message, e.response?.data));
