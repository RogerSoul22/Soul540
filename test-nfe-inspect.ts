...
function main() {
  ...
  console.log('\n=== ESTRUTURA DO NFe ASSINADO ===');
  // Mostrar apenas as tags principais
  const tags = signed.match(/<[^/!?][^>]*>/g) ?? [];
  const unique = [...new Set(tags.map(t => t.replace(/ .*>/, '>')))];
  unique.forEach(t => console.log(t));

  console.log('\n=== INÍCIO DO XML ASSINADO ===');
