    ...
    <div id="menu-pdf" style="font-family:Arial,sans-serif;background:#fff;color:#1a1a1a;padding:0;margin:0;width:210mm;">
      <style>
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .item{padding-bottom:14px;border-bottom:1px solid #e5e7eb;} .item:last-child{border-bottom:none;}
        .item-header{display:flex;flex-wrap:wrap;align-items:baseline;gap:6px;margin-bottom:4px;}
        .item-name{font-size:13px;font-weight:700;color:#111;} .item-sub{font-size:11px;color:#6b7280;font-style:italic;}
        .item-desc{font-size:12px;line-height:1.65;color:#374151;}
        .item-harm{font-size:11px;color:#9ca3af;font-style:italic;margin-top:5px;} .item-harm strong{color:#6b7280;font-style:normal;}
...
export default function Cardapios() {
  ...
  const [staticMenuOverrides, setStaticMenuOverrides] = useState<Record<string, StaticMenu>>(() => {
    try { return JSON.parse(localStorage.getItem('soul540_franchise_menu_overrides') || '{}'); } catch { return {}; }
  });
  ...
  const saveStaticOverride = (menu: StaticMenu) => {
    ...
    localStorage.setItem('soul540_franchise_menu_overrides', JSON.stringify(next));
    ...
    <div className={styles.page}>
      ...
      <div className={styles.standardGrid}>
        {SOUL540_MENUS.map((menu) => {
          ...
          const totalPizzas = activeMenu.categories.find(c => c.name.toLowerCase().includes('salgad') || c.name.toLowerCase().includes('tradicional'))?.items.length || 0;
          return (
            <div key={menu.id} className={`${styles.standardCard} ${colorClass}`}>
                ...
                <div className={styles.standardStatsRow}>
                  {activeMenu.categories.map((cat) => (
                    <span key={cat.name} className={styles.standardStat}>
                      <strong>{cat.items.length}</strong> {cat.name.toLowerCase().includes('entrada') ? 'entradas' : isSweet(cat) ? 'doces' : 'sabores'}
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div className={styles.standardCardFooter}>
                ...
                <button className={styles.btnPdf} onClick={() => handleGeneratePdf(activeMenu)} disabled={generatingPdf === menu.id} title="Gerar PDF">
        ...
        <div className={styles.overlay} onClick={() => setPreviewStatic(null)}>
          <div className={styles.menuOverlay} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.menuTopActions}>
              <button className={styles.btnPdfModal} onClick={() => handleGeneratePdf(previewStatic)} disabled={generatingPdf === previewStatic.id}>
            ...
            <div className={styles.menuDocBody}>
              {previewStatic.categories.map((cat) => (
                <div key={cat.name} className={styles.menuDocSection}>
                  <div className={`${styles.menuDocCatTitle} ${isSweet(cat) ? styles.menuDocCatSweet : ''}`}>{cat.name}</div>
                  <div className={styles.menuDocItems}>
                    {cat.items.map((item) => (
                      <div key={item.name} className={styles.menuDocItem}>
                        ...
                        {item.harmonization && <p className={styles.menuDocItemHarm}><strong>Harmonização:</strong> {item.harmonization}</p>}
                      </div>
                    ))}
                  </div>
                </div>
