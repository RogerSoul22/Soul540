...
export default function Tarefas() {
  ...
  const handleMoveToHistory = async (task: (typeof tasks)[0]) => {
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <div className={styles.headerActions}>
          ...
          <button className={styles.btnSecondary} onClick={() => window.open(`/campinas${ROUTES.CHECKLIST_ENVIO}`, '_blank', 'noopener,noreferrer')}>
              ...
              taskHistory.map((entry) => (
                <div key={entry.id} className={styles.historyItem}>
                  ...
                  <div className={styles.historyItemActions}>
                    <Badge variant={priorityColors[entry.priority as TaskPriority] || 'gray'}>{priorityLabels[entry.priority as TaskPriority] || entry.priority}</Badge>
                    <button
                      className={styles.historyReopenBtn}
                      onClick={async () => {
                        await addTask({ title: entry.title, description: entry.description, status: 'todo', priority: entry.priority as TaskPriority, assignee: entry.assignee, dueDate: entry.dueDate, eventId: entry.eventId });
