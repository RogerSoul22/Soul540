...
const emptyForm: FormData = {
  name: '', role: 'auxiliar', phone: '', cpf: '', rg: '', address: '', notes: '', pixKey: '', commissionRate: '0', availableDays: [], status: 'ativo',
};
...
export default function Funcionarios() {
  ...
  const openEdit = (emp: Employee) => {
    setForm({
      ...
      notes: emp.notes ?? '', pixKey: emp.pixKey ?? '', commissionRate: String(emp.commissionRate ?? 0), availableDays: emp.availableDays ?? [],
    });
    setEditingId(emp.id);
    setShowModal(true);
  };
