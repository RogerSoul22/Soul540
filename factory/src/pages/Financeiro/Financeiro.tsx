...
const formCategories: Record<FinanceType, string[]> = {
  revenue: ['contrato', 'adicional', 'equipe', 'ingredientes', 'logistica', 'outros'],
...
export default function Financeiro() {
  ...
  const fixedCosts = useMemo(() => {
    ...
    return Object.entries(costs)
      .map(([cat, val]) => ({ label: CATEGORY_LABELS[cat] || cat, value: val, color: CATEGORY_COLORS[cat] || '#64748b' }))
  ...
  const insumosRatio = useMemo(() => {
    const insumos = pageMonthFinances.filter((f) => f.type === 'cost' && f.category === 'insumos').reduce((acc, f) => acc + f.amount, 0);
    return totalRevenue > 0 ? (insumos / totalRevenue) * 100 : 0;
  }, [pageMonthFinances, totalRevenue]);

  const maoObraRatio = useMemo(() => {
    const mo = pageMonthFinances.filter((f) => f.type === 'cost' && (f.category === 'salario' || f.category === 'pro-labore')).reduce((acc, f) => acc + f.amount, 0);
    return totalRevenue > 0 ? (mo / totalRevenue) * 100 : 0;
  }, [pageMonthFinances, totalRevenue]);

  const insumosRatioMensal = useMemo(() => {
    const insumos = monthFinances.filter((f) => f.type === 'cost' && f.category === 'insumos').reduce((acc, f) => acc + f.amount, 0);
    return monthRevenue > 0 ? (insumos / monthRevenue) * 100 : 0;
  }, [monthFinances, monthRevenue]);

  const maoObraRatioMensal = useMemo(() => {
    const mo = monthFinances.filter((f) => f.type === 'cost' && (f.category === 'salario' || f.category === 'pro-labore')).reduce((acc, f) => acc + f.amount, 0);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `financeiro-fabrica-${filterMonth === 'all' ? 'todos' : filterMonth}.csv`;
    a.click();
    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <p className={styles.subtitle}>Controle de receitas e despesas da Fábrica</p>
        </div>
        ...
        <div className={styles.tabContent}>
            ...
            <div className={styles.chartContainer}>
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={monthlyData} barGap={4} barCategoryGap="15%">
                  ...
                  <XAxis dataKey="month" tickFormatter={formatMonth} tick={{ fill: '#64748b', fontSize: 12 }} axisLine={{ stroke: 'rgba(255,255,255,0.1)' }} tickLine={false} />
                  <YAxis tickFormatter={(v: number) => `${(v / 1000).toFixed(0)}k`} tick={{ fill: '#64748b', fontSize: 12 }} axisLine={false} tickLine={false} />
                  <Tooltip
                    contentStyle={{ background: '#1a2235', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#f1f5f9', fontSize: '13px' }}
          ...
          <div className={styles.gaugesGrid}>
            <div className={styles.gaugeCard}>
              <GaugeChart label="Margem de Lucro" value={margin} max={100} suffix="%" color={margin >= 20 ? '#4ade80' : margin >= 10 ? '#fbbf24' : '#f87171'} />
            </div>
            <div className={styles.gaugeCard}>
              <GaugeChart label="Insumos / Receita" value={insumosRatio} max={100} suffix="%" color={insumosRatio <= 30 ? '#4ade80' : insumosRatio <= 45 ? '#fbbf24' : '#f87171'} />
            </div>
            <div className={styles.gaugeCard}>
              <GaugeChart label="Mao de Obra / Receita" value={maoObraRatio} max={100} suffix="%" color={maoObraRatio <= 25 ? '#4ade80' : maoObraRatio <= 40 ? '#fbbf24' : '#f87171'} />
            </div>
          </div>
        </div>
      )}
        ...
        <div className={styles.tabContent}>
          ...
          <div className={styles.chartSection}>
            ...
            {(() => {
              ...
              const totals = cats
                .map((cat) => ({ cat, total: pageMonthFinances.filter((f) => f.type === 'cost' && f.category === cat).reduce((a, f) => a + f.amount, 0) }))
      ...
      {activeTab === 'mensal' && (
        <div className={styles.tabContent}>
          ...
          <div className={styles.gaugesGrid}>
            <div className={styles.gaugeCard}>
              <GaugeChart label="Lucro" value={monthMargin} max={100} suffix="%" color={monthMargin >= 20 ? '#4ade80' : monthMargin >= 10 ? '#fbbf24' : '#f87171'} />
            </div>
            <div className={styles.gaugeCard}>
              <GaugeChart label="Insumos" value={insumosRatioMensal} max={100} suffix="%" color={insumosRatioMensal <= 30 ? '#4ade80' : insumosRatioMensal <= 45 ? '#fbbf24' : '#f87171'} />
            </div>
            <div className={styles.gaugeCard}>
              <GaugeChart label="Mao de Obra" value={maoObraRatioMensal} max={100} suffix="%" color={maoObraRatioMensal <= 25 ? '#4ade80' : maoObraRatioMensal <= 40 ? '#fbbf24' : '#f87171'} />
            </div>
          </div>

          <div className={styles.barsGrid}>
            <div className={styles.barCard}>
              <HorizontalBarChart title="Despesas Variaveis" items={variableCosts} formatValue={formatBRL} />
            </div>
            <div className={styles.barCard}>
              <HorizontalBarChart title="Despesas Fixas" items={fixedCosts} formatValue={formatBRL} />
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'lancamentos' && (
        <div className={styles.tabContent}>
          <div className={styles.filterRow}>
            ...
            <select className={styles.searchInput} value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)}>
              <option value="all">Todos os meses</option>
              {availableMonths.map((m) => (
                <option key={m} value={m}>{formatMonth(m)}</option>
              ))}
                    ...
                    <tr key={entry.id}>
                      ...
                      <td>
                        <span className={styles.categoryTag}>
                          <span className={styles.categoryDot} style={{ background: CATEGORY_COLORS[entry.category] || '#64748b' }} />
          ...
          <form className={styles.form} onSubmit={handleSubmit}>
            <div className={styles.formRow}>
              <div className={styles.formField}>
                ...
                <select
                  ...
                  value={formType}
                  onChange={(e) => { setFormType(e.target.value as FinanceType); setFormCategory(''); }}
                >
              ...
              <div className={styles.formField}>
                ...
                <select className={styles.formSelect} value={formEventId} onChange={(e) => setFormEventId(e.target.value)}>
            ...
            <div className={styles.formRow}>
              <div className={styles.formField}>
                ...
                <select className={styles.formSelect} value={formCategory} onChange={(e) => setFormCategory(e.target.value)} required>
                  <option value="">Selecione...</option>
            ...
            <div className={styles.formRow}>
              ...
              <div className={styles.formField}>
                ...
                <select className={styles.formSelect} value={formStatus} onChange={(e) => setFormStatus(e.target.value as FinanceStatus)}>
