...
export default function Financeiro() {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `financeiro-fabrica-${financePeriod.start || 'todos'}_a_${financePeriod.end || 'todos'}.csv`;
    a.click();
  ...
  const exportDRE = () => {
    const availablePeriodMonths = [...new Set(
      periodFinances.map((finance) => finance.date?.slice(0, 7)).filter(Boolean) as string[],
    )].sort();
        ...
        <div className={styles.tabContent}>
          ...
          <div className={styles.chartSection}>
            ...
            {(() => {
              ...
              if (totals.length === 0) return <p className={styles.emptyState}>Nenhuma despesa no período selecionado.</p>;
              return (
                  ...
                  <div className={styles.formField}>
                    ...
                    {formRecurring && (
                      <div className={styles.recurrenceConfig}>
                        ...
                        <div className={styles.recurrenceSummary}><strong>Recorrências previstas</strong><span>{recurrencePreview.length} lançamentos · {formatBRL(parseCurrency(formAmount) * recurrencePreview.length)}</span><small>{recurrencePreview.slice(0, 4).map((date) => new Date(`${date}T12:00:00`).toLocaleDateString('pt-BR')).join(' · ')}</small></div>
                      </div>
                    )}
                  </div>
                </div>
